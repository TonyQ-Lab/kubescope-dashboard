...
import { KVRow, Section, TolerationRow } from "../../components/Modals";

const PANEL_MIN_HEIGHT = 150;
const PANEL_MAX_HEIGHT = 600;
const PANEL_DEFAULT_HEIGHT = 380;

function StatusBadge({ numberReady, desiredNumberScheduled }) {
  const allReady = numberReady >= desiredNumberScheduled && desiredNumberScheduled > 0;
  const partial = numberReady > 0 && numberReady < desiredNumberScheduled;
  const cls = allReady
...
function NodeSpreadBar({ ready = 0, scheduled = 0, desired = 0 }) {
  const pct = desired > 0 ? Math.round((ready / desired) * 100) : 0;
  const misscheduled = scheduled - desired;
  return (
    <div className="px-4 py-1.5">
      <div className="flex items-center justify-between mb-1.5">
        <span className="text-md text-gray-500">Node coverage</span>
        <span className="text-md font-mono text-gray-200">
          {ready}/{desired} ready
          {misscheduled > 0 && (
            <span className="text-yellow-400 ml-2">· {misscheduled} misscheduled</span>
          )}
        </span>
      </div>
  ...
  return (
    <div className="mx-4 mb-2 rounded-md border border-gray-700/60 bg-gray-800/30 px-3 py-2">
      ...
      {container.securityContext && (
        <div className="mt-1.5 flex gap-3 flex-wrap text-md font-mono">
          {container.securityContext.privileged && (
            <span className="bg-red-500/15 text-red-400 px-1.5 py-0.5 rounded">privileged</span>
          )}
          {container.securityContext.readOnlyRootFilesystem && (
            <span className="bg-gray-700/50 text-gray-400 px-1.5 py-0.5 rounded">readOnlyRootFS</span>
          )}
          {container.securityContext.runAsUser != null && (
            <span className="bg-gray-700/50 text-gray-400 px-1.5 py-0.5 rounded">
        ...
        <div className="mt-1.5 space-y-0.5">
          {container.volumeMounts.map((vm, i) => (
            <div key={i} className="flex gap-2 text-md font-mono">
              <span className="text-gray-500 shrink-0">{vm.name}</span>
              <span className="text-gray-400 truncate">{vm.mountPath}</span>
              {vm.readOnly && <span className="text-gray-600 shrink-0">ro</span>}
            </div>
          ))}
        </div>
      )}
...
export default function DaemonDetails({ daemonSet, onClose }) {
  ...
  const updatedScheduled = status?.updatedNumberScheduled ?? 0;

  const updateStrategy = spec?.updateStrategy?.type ?? "—";
  const rollingUpdate = spec?.updateStrategy?.rollingUpdate;

  const nodeSelector = spec?.template?.spec?.nodeSelector
    ? Object.entries(spec.template.spec.nodeSelector)
    : [];
  ...
  const tabs = ["overview", "containers", "scheduling", "strategy", "labels"];
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-gray-700/70 shrink-0">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <StatusBadge numberReady={ready} desiredNumberScheduled={desired} />
        </div>
        <button
          onClick={handleClose}
      ...
      <div className="flex-1 overflow-y-auto py-3">
        ...
        {tab === "overview" && (
          <>
            <Section title="Node coverage">
              <NodeSpreadBar ready={ready} scheduled={current} desired={desired} />
              <KVRow label="Desired" value={String(desired)} />
              <KVRow label="Current" value={String(current)} />
              <KVRow label="Ready" value={String(ready)} />
              <KVRow label="Available" value={String(available)} />
              {unavailable > 0 && (
                <KVRow label="Unavailable" value={String(unavailable)} />
              )}
              {misscheduled > 0 && (
                <KVRow label="Misscheduled" value={String(misscheduled)} />
              )}
              <KVRow label="Updated" value={String(updatedScheduled)} />
            </Section>
        ...
        {tab === "scheduling" && (
          <>
            <Section title={`Node selector (${nodeSelector.length})`}>
              {nodeSelector.length ? (
                nodeSelector.map(([k, v]) => <KVRow key={k} label={k} value={v} />)
              ) : (
                <p className="px-4 text-md text-gray-500">Runs on all nodes</p>
              )}
            </Section>
            <Section title={`Tolerations (${tolerations.length})`}>
              {tolerations.length ? (
                tolerations.map((t, i) => <TolerationRow key={i} toleration={t} />)
              ) : (
                <p className="px-4 text-md text-gray-500">No tolerations</p>
              )}
            </Section>
            {spec?.template?.spec?.affinity && (
              <Section title="Affinity">
                <p className="px-4 text-md text-gray-400 font-mono">
                  {Object.keys(spec.template.spec.affinity).join(", ")}
                </p>
              </Section>
            )}
          </>
        ...
        {tab === "strategy" && (
          <>
            <Section title="Update strategy">
              <KVRow label="Type" value={updateStrategy} />
              {rollingUpdate && (
                <KVRow
                  label="Max unavailable"
                  value={String(rollingUpdate.maxUnavailable ?? "—")}
                />
              )}
              <KVRow
                label="Min ready sec."
