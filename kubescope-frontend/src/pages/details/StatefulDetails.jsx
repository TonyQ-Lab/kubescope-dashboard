...
function ReplicaBar({ ready = 0, desired = 1, label = "Pod health" }) {
        ...
        <div className="mt-1.5 space-y-0.5">
          {container.volumeMounts.map((vm, i) => (
            <div key={i} className="flex gap-2 text-md font-mono">
              ...
              {vm.readOnly && (
                <span className="text-gray-600 shrink-0">ro</span>
              )}
            </div>
          ))}
        </div>
...
function VolumeClaimTemplateCard({ vct }) {
  const spec = vct.spec ?? {};
  const requests = spec.resources?.requests?.storage;
  const modes = spec.accessModes?.join(", ");
  return (
    <div className="mx-4 mb-2 rounded-md border border-gray-700/60 bg-gray-800/30 px-3 py-2">
      <div className="flex items-center justify-between mb-1">
        <span className="text-md font-medium text-gray-200">{vct.metadata?.name ?? "—"}</span>
        {requests && (
          <span className="text-md font-mono text-gray-400">{requests}</span>
        )}
      </div>
      {spec.storageClassName && (
        <div className="text-md text-gray-500 font-mono truncate">
          StorageClass: {spec.storageClassName}
        </div>
      )}
      {modes && (
        <div className="text-md text-gray-500 font-mono mt-0.5">
          AccessModes: {modes}
        </div>
      )}
    </div>
...
export default function StatefulDetails({ statefulSet, onClose }) {
  ...
  const tabs = ["overview", "containers", "storage", "strategy", "labels"];
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-gray-700/70 shrink-0">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <StatusBadge ready={readyReplicas} desired={desired} />
        </div>
        <button
          onClick={handleClose}
      ...
      <div className="flex-1 overflow-y-auto py-3">
        ...
        {tab === "overview" && (
          <>
            <Section title="Pods">
              <ReplicaBar ready={readyReplicas} desired={desired} label="Pod health" />
              <KVRow label="Desired" value={String(desired)} />
              <KVRow label="Current" value={String(currentReplicas)} />
              <KVRow label="Ready" value={String(readyReplicas)} />
              <KVRow label="Updated" value={String(updatedReplicas)} />
              <KVRow label="Current revision" value={status?.currentRevision} />
              <KVRow label="Update revision" value={status?.updateRevision} />
            </Section>
            <Section title="Metadata">
              ...
              <KVRow label="Service name" value={spec?.serviceName} />
              <KVRow label="Pod mgmt. policy" value={podManagementPolicy} />
            </Section>
          </>
        )}
        ...
        {tab === "storage" && (
          <>
            <Section title={`Volume claim templates (${vcts.length})`}>
              {vcts.length ? (
                vcts.map((vct, i) => (
                  <VolumeClaimTemplateCard key={vct.metadata?.name ?? i} vct={vct} />
                ))
              ) : (
                <p className="px-4 text-md text-gray-500">No volume claim templates</p>
              )}
            </Section>
            {spec?.template?.spec?.volumes?.length > 0 && (
              <Section title={`Volumes (${spec.template.spec.volumes.length})`}>
                {spec.template.spec.volumes.map((v) => {
                  const sourceType = Object.keys(v).find((k) => k !== "name") ?? "—";
                  return <KVRow key={v.name} label={v.name} value={sourceType} />;
                })}
              </Section>
            )}
          </>
